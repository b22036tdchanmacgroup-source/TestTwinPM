...
function getSampleData() {
  ...
  return [
    { depth1: '구조분야:BridgePlanner', depth2: '1.기획·기술 조사', depth3: 'A. S/W 기획', depth4: 'A-1. 제품 기획서', manager: '한이', filename: '01 [개요]개발이력_25.11.03.xlsx', author: '맥이', reviewer: '조태희', revStatus: '완료', confirmer: '권혁진', confStatus: '확인중', date: '2026.04.21', size: '80.5 KB', status: '승인' },
    { depth1: '구조분야:BridgePlanner', depth2: '1.기획·기술 조사', depth3: 'A. S/W 기획', depth4: 'A-1. 제품 기획서', manager: '한이', filename: '02 [일반사항] 교량설계 아이템 분류_24.06.14.xlsx', author: '맥이', reviewer: '조태희', revStatus: '완료', confirmer: '권혁진', confStatus: '완료', date: '2026.04.21', size: '21.4 KB', status: '승인' },
    { depth1: '수리분야:RiverFlow', depth2: '1.기획·기술 조사', depth3: 'A. S/W 기획', depth4: 'A-1. 제품 기획서', manager: '김이', filename: '수리해석_가이드.pdf', author: '최이', reviewer: '김우진A', revStatus: '검토중', confirmer: '성형일', confStatus: '-', date: '2026.04.22', size: '1.2 MB', status: '승인' },
    { depth1: '총괄기획:Migration Simulator', depth2: '1.현황분석', depth3: 'A.시스템분석', depth4: 'A-1 분석자료', manager: '박이', filename: '마이그레이션_전략_v1.0.docx', author: '이이', reviewer: '김우진A', revStatus: '보류', confirmer: '권혁진', confStatus: '-', date: '2026.04.25', size: '45.2 KB', status: '진행' },
    { depth1: '기획실:ERP Migration', depth2: '1.현황분석', depth3: 'A.데이터분석', depth4: 'A-1 데이터맵', manager: '정이', filename: 'ERP_연동_정의서.xlsx', author: '권이', reviewer: '조태희', revStatus: '-', confirmer: '성형일', confStatus: '-', date: '2026.04.26', size: '92.1 KB', status: '승인' }
  ];
}
...
const SW_CONFIG = {
  "HmEG(HmDraw)": "D", "EG-BIM Modeler": "D", "EG-BIM Drawer": "D", "StrAna": "D", "STRANA": "D",
  "천지인": "B", "GAIA": "B", "KNGIL": "B", "Survey Planner": "B", "GIS Mapper": "B", "Cadaster": "B", "Domainer": "B",
  "BridgePlanner": "A", "DRZainer": "A", "BriZainer-DR": "A", "NodularZainer": "A", "BriZainer-Nodular": "A", 
  "AbutZainer": "A", "PierZainer": "A", "BoxZainer": "A", "WallZainer": "A", "TunnelZainer": "A", "TunnerZainer": "A", "CulvertZainer": "A",
  ...
  "bCMf": "C", "GSIM": "C", "CCP": "C", "단가/공정 solution": "A", "단가/공정 솔루션": "A",
  "철도 플랫폼 개발": "B", "Urban Analyzer": "B", "Pile Analyzer": "B"
};
...
function selectSubCategory(catKey, subKey) {
  ...
  files.forEach((file, idx) => {
    ...
    tr.innerHTML = `
      ...
      <td>${file.author}</td>
      <td>${file.author}</td>
      <td>${renderStatusBadge(file.reviewer, file.revStatus)}</td>
      <td>${renderStatusBadge(file.confirmer, file.confStatus)}</td>
      <td>${formattedDate}</td>
      <td>${file.size}</td>
...
function renderStatusBadge(name, status) {
  if (!name || name === '-' || status === '-') return '<span class="badge-status none">-</span>';
  
  let cls = 'process';
  let label = status || '검토중';

  if (status === '완료') cls = 'done';
  else if (status === '보류') cls = 'hold';
  else if (status === '검토중' || status === '확인중') cls = 'process';
  else if (status === '대기') cls = 'pending';

  return `
    <div class="badge-status ${cls}" onclick="simulateStatusChange(this, '${name}')">
      <span class="status-dot"></span>
      <span class="status-label">${label}</span>
      <span class="status-name" style="margin-left:5px; opacity:0.8; font-weight:400;">${name}</span>
    </div>
  `;
}

window.simulateStatusChange = function(el, name) {
  const labelEl = el.querySelector('.status-label');
  const current = labelEl.textContent;
  
  if (current.includes('중')) {
    el.className = 'badge-status done';
    labelEl.textContent = '완료';
  } else if (current === '완료') {
    el.className = 'badge-status hold';
    labelEl.textContent = '보류';
  } else {
    el.className = 'badge-status process';
    labelEl.textContent = current.includes('검토') ? '검토중' : '확인중';
  }
};
...
function calculateMetrics(data) {
  ...
  // 1. 중복 관리 (Duplicates)
  const fileNames = data.map(f => f.filename);
  const duplicateFiles = fileNames.filter((name, index) => fileNames.indexOf(name) !== index);
  const duplicateScore = total > 0 ? ((total - duplicateFiles.length) / total) * 100 : 100;
  
  // 2. 파일 건전성 (Health)
  const zeroFiles = data.filter(f => f.size === '0 KB' || f.size === '0 B' || f.size === '0').length;
  const tempFiles = data.filter(f => f.filename.endsWith('.tmp') || f.filename.endsWith('.bak')).length;
  const healthScore = total > 0 ? ((total - (zeroFiles + tempFiles)) / total) * 100 : 100;

  // 3. 접근성 (Accessibility/Hierarchy)
  const accessibilityScore = Math.min(100, (Object.keys(depth3Map).length / 5) * 100);
  ...
  if (duplicateFiles.length > 0) {
    recommendations.push({ 
      ...
      icon: '👯', 
      title: '중복 문서 검출', 
      text: `동일한 파일명을 가진 문서가 ${duplicateFiles.length}건 발견되었습니다. 중복 배치를 확인하세요.` 
    });
  }
  ...
  if (zeroFiles > 0) {
    recommendations.push({ 
      ...
      icon: '🚫', 
      title: '내용 없는 파일', 
      text: `크기가 0인 파일이 ${zeroFiles}건 존재합니다. 업로드 오류 여부를 확인하세요.` 
    });
  }
      ...
      <div class="report-body" style="flex:1; overflow-y:auto; padding-right:10px; padding-bottom:30px;">
        ...
        <div class="report-section" style="display:flex; gap: 20px; flex-wrap: wrap;">
          <div style="flex:1; min-width:200px;">
              ...
              ${renderMetric('중복 관리', m.duplicateScore, '동일 파일 중복 배치 검출')}
              ${renderMetric('파일 건전성', m.healthScore, '0바이트/임시 파일 무결성')}
              ${renderMetric('접근성/체계', m.accessibilityScore, '표준 체계 및 분산 적정성')}
          </div>
        </div>
...
function showFolderTree(type) {
  ...
  // Standard Tree View for all types
...
function openPresidentReport() {
  const panel = document.getElementById('president-report-panel');
  panel.classList.add('open');
  loadPresidentReports();
}

function closePresidentReport() {
  const panel = document.getElementById('president-report-panel');
  panel.classList.remove('open');
}

function loadPresidentReports() {
  const reports = [
    { date: '2026.05.06', title: '20260506_1차 보고', file: '20260506_1차 보고.pdf' },
    { date: '2026.05.12', title: '20260512_2차 보고', file: '20260512_2차 보고.pdf' }
  ];
  
  const container = document.getElementById('report-date-tabs');
  container.innerHTML = '';
  
  reports.forEach((report, i) => {
    const tab = document.createElement('div');
    tab.className = 'report-date-tab';
    tab.innerHTML = `<span>${report.date}</span><div style="font-size:10px; opacity:0.7;">${report.title.split('_')[1]}</div>`;
    tab.onclick = () => selectPresidentReport(report, tab);
    container.appendChild(tab);
    
    if (i === reports.length - 1) { // Select latest by default
       selectPresidentReport(report, tab);
    }
  });
}

function selectPresidentReport(report, tabElement) {
  document.querySelectorAll('.report-date-tab').forEach(t => t.classList.remove('active'));
  tabElement.classList.add('active');
  
  const frame = document.getElementById('report-pdf-frame');
  const placeholder = document.getElementById('pdf-viewer-placeholder');
  
  // Use absolute path for local files if needed, or relative to workspace
  // Assuming '사장님 보고문서' folder is in the same directory as index.html
  frame.src = `./사장님 보고문서/${report.file}`;
